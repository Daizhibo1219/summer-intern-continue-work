...
#加load_offset,load_scale,load_moving_mean,load_moving_variance,load_mark
def Batchnorm(name, axes, inputs,
              load_offset,load_scale,load_moving_mean,load_moving_variance,load_mark=False,
    ...
    if ((axes == [0,2,3]) or (axes == [0,2])) and fused==True:
        ...
        if load_mark == True:
            offset = lib.param(name + '.offset', load_offset)
            scale = lib.param(name + '.scale', load_scale)

            moving_mean = lib.param(name + '.moving_mean', load_moving_mean,
                                    trainable=False)
            moving_variance = lib.param(name + '.moving_variance', load_moving_variance,
                                        trainable=False)
        else:
        ...
        if 0 not in axes:
            print ("WARNING ({}): didn't find 0 in axes, but not using separate BN params for each item in batch".format(name))#改了print
            shape[0] = 1
        if load_mark == True:
            offset = lib.param(name + '.offset', load_offset)
            scale = lib.param(name + '.scale', load_scale)
